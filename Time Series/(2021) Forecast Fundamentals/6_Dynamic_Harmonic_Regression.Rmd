...
# K = 1
fit_dhx1 <- auto.arima(tsC_train[,'Y'], xreg = cbind(tsC_train[,c('Xc', 'Xd')], fourier(tsC_train[,'Y'], K = 1)), seasonal=F)
fc_dhx1 <- forecast(fit_dhx1, xreg = cbind(tsC_test[,c('Xc', 'Xd')], fourier(tsC_train[,'Y'], K = 1, h=12)), level=c(0,0))
fci_dhx1 <- Y_inv(fc_dhx1)
autoplot(fci_dhx1) + autolayer(tsCs_test)
```

<br>
**_K=6_**
```{r}
fit_dhx6 <- auto.arima(tsC_train[,'Y'], xreg = cbind(tsC_train[,c('Xc', 'Xd')], fourier(tsC_train[,'Y'], K = 6)), seasonal=F)
fc_dhx6 <- forecast(fit_dhx6, xreg = cbind(tsC_test[,c('Xc', 'Xd')], fourier(tsC_train[,'Y'], K = 6, h=12)), level=c(0,0))
fci_dhx6 <- Y_inv(fc_dhx6)
autoplot(fci_dhx6) + autolayer(tsCs_test)
```
...
get_dhx_AICc <- function(K) {
  fit_dhx <- auto.arima(tsC_train[,'Y'], xreg = cbind(tsC_train[,c('Xc', 'Xd')], fourier(tsC_train[,'Y'], K = K)), seasonal=F)
  return(fit_dhx$aicc)
}
...
dhx1C_is <- seq(nrow(tsC_train)/2, nrow(tsC_train)-12)
dhx1C_tscv <- data.frame(i = dhx1C_is, rmse = sapply(dhx1C_is, get_dhx1i_rmse))

# Performance
fci_dhx1_rmse <- rmse(as.numeric(fci_dhx1$mean), tsCs_actual)
fci_dhx1_mase <- mase(as.numeric(fci_dhx1$mean), tsCs_actual)
dhx1_tscv_rmse <- round(mean(dhx1C_tscv$rmse), digits=2)
