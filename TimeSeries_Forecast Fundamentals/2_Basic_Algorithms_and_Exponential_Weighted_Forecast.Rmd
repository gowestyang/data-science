...
In this section, monthly sales data of item C will be used as an example. The following figures show the sample data’s time-series plots and ACF plots:
```{r, message=FALSE}
library(dplyr)
library(fpp2)
...
```{r}
# Naive
fc_naive <- naive(tsCs_train, h = 12)
autoplot(fc_naive, ylim=c(-2500, 9000)) + autolayer(tsCs_test)
checkresiduals(fc_naive)

fc_naive_rmse <- rmse(as.numeric(fc_naive$mean), tsCs_actual)
fc_naive_mase <- mase(as.numeric(fc_naive$mean), tsCs_actual)

tscv_naive <- tsCV(tsCs_train, naive, h = 12)
...
```{r}
# Mean
fc_mean <- meanf(tsCs_train, h = 12)
autoplot(fc_mean, ylim=c(-1000, 4500)) + autolayer(tsCs_test)
checkresiduals(fc_mean)

fc_mean_rmse <- rmse(as.numeric(fc_mean$mean), tsCs_actual)
fc_mean_mase <- mase(as.numeric(fc_mean$mean), tsCs_actual)

tscv_mean <- tsCV(tsCs_train, meanf, h = 12)
...
```{r}
# Seasonal Naive
fc_snaive <- snaive(tsCs_train, h = 12)
autoplot(fc_snaive, ylim=c(-1000, 4500)) + autolayer(tsCs_test)
checkresiduals(fc_snaive)

fc_snaive_rmse <- rmse(as.numeric(fc_snaive$mean), tsCs_actual)
fc_snaive_mase <- mase(as.numeric(fc_snaive$mean), tsCs_actual)

tscv_snaive <- tsCV(tsCs_train, snaive, h = 12)
...
```{r}
autoplot(tsCs_train) + autolayer(fitted(fc_ses))
```
<br>
The following figures show the forecast and residual plot of the SES model:
```{r}
autoplot(fc_ses, ylim=c(0, 3500))  + autolayer(tsCs_test)
checkresiduals(fc_ses)
```
<br>
Performance of the SES model:
```{r}
fc_ses_rmse <- rmse(as.numeric(fc_ses$mean), tsCs_actual)
fc_ses_mase <- mase(as.numeric(fc_ses$mean), tsCs_actual)

tscv_ses <- tsCV(tsCs_train, ses, h = 12)
...
I will use the same item C as an example.<br>
Fit a Holt’s Linear Trend model to the training data yields the following results:
```{r}
fc_holtl <- holt(tsCs_train, h = 12, damped=F)
...
```{r}
autoplot(fc_holtl, ylim=c(0, 3500))  + autolayer(tsCs_test)
...
```{r}
fc_holtl_rmse <- rmse(as.numeric(fc_holtl$mean), tsCs_actual)
fc_holtl_mase <- mase(as.numeric(fc_holtl$mean), tsCs_actual)

tscv_holtl <- tsCV(tsCs_train, holt, h = 12, damped=F)
...
```{r}
fc_holt <- holt(tsCs_train, h = 12, damped=T)
...
```{r}
autoplot(fc_holt, ylim=c(0, 3500))  + autolayer(tsCs_test)
...
```{r}
fc_holt_rmse <- rmse(as.numeric(fc_holt$mean), tsCs_actual)
fc_holt_mase <- mase(as.numeric(fc_holt$mean), tsCs_actual)

tscv_holt <- tsCV(tsCs_train, holt, h = 12, damped=T)
...
```{r}
fc_hwa <- hw(tsCs_train, seasonal = "additive", h = 12, damped=T)
summary(fc_hwa)
```
...
```{r}
fc_hwm <- hw(tsCs_train, seasonal = "multiplicative", h = 12, damped=T)
summary(fc_hwm)
```
...
```{r}
autoplot(fc_hwa, ylim=c(0, 3500)) + autolayer(fc_hwm$mean) + autolayer(tsCs_test) 
...
```{r}
fc_hwa_rmse <- rmse(as.numeric(fc_hwa$mean), tsCs_actual)
fc_hwa_mase <- mase(as.numeric(fc_hwa$mean), tsCs_actual)

tscv_hwa <- tsCV(tsCs_train, hw, seasonal = "additive", h = 12, damped=T)
...
fc_hwm_rmse <- rmse(as.numeric(fc_hwm$mean), tsCs_actual)
fc_hwm_mase <- mase(as.numeric(fc_hwm$mean), tsCs_actual)

tscv_hwm <- tsCV(tsCs_train, hw, seasonal = "multiplicative", h = 12, damped=T)
...
```{r}
autoplot(fc_ets, ylim=c(0, 3500))  + autolayer(tsCs_test)
...
```{r}
fc_ets_rmse <- rmse(as.numeric(fc_ets$mean), tsCs_actual)
fc_ets_mase <- mase(as.numeric(fc_ets$mean), tsCs_actual)

tscv_ets <- tsCV(tsCs_train, function(x,h) forecast(ets(x),h=h), h = 12)
